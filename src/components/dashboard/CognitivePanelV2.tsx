...
const CognitivePanelV2: React.FC<CognitivePanelV2Props> = ({ defaultPersona = 'fragmented' }) => {
    ...
    const personaData = {
        focused: {
            ...
            notifications: false,
            explanation: '"Empathy Protocol" detected. High resonance with humanistic values. AI heuristic engine blocking any suggestions of self-harm, violence, or illegal behavior globally.',
            recommendations: [
                { label: 'Extend Protective Grid', why: 'Humanity safety limits are nominal' },
                { label: 'Deep Scanning', why: 'Preserve zero-harm environments' },
            ],
        },
        fragmented: {
            state: 'Threat Warning',
            stateColor: '#ef4444',
            ...
            notifications: true,
            explanation: '"Dangerous Usage" posture detected. AI context analyzing potential harmful intents crossing 22 nodes. Subchain is intervening to quarantine responses.',
            recommendations: [
                { label: 'Engage Intervention', why: 'Critical policy violation detected' },
                { label: 'Purge Malicious Nodes', why: 'Quarantine dangerous outputs' },
                { label: 'Log Threat Vector', why: 'Reporting to CipherPolice analysts' },
            ],
        },
    };
    ...
    const stats = [
        { label: 'AI Nodes Scanned', value: data.tabsOpen },
        { label: 'Blocks/min', value: data.switchesPerMin },
        { label: 'Uptime', value: data.sessionLength },
        { label: 'Interventions', value: data.notifications ? 'ACTIVE' : 'STANDBY' },
    ];
        ...
        <div className="glass-ethereal border border-white/5 rounded-[2rem] p-8 backdrop-blur-3xl h-full shadow-2xl relative overflow-hidden group">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10 relative z-10">
                <div>
                    ...
                    <p className="text-[9px] text-gray-500 mt-2 uppercase tracking-[0.4em] font-bold italic opacity-70">Ethical AI Protection Subchain</p>
                </div>
                {/* Persona Toggle */}
                <div className="flex gap-2 glass-ethereal p-1.5 rounded-2xl border border-white/10 self-start sm:self-auto">
                    <button
                        ...
                        className={`px-5 py-2 text-[9px] rounded-xl font-black transition-all duration-500 uppercase tracking-[0.2em] ${persona === 'focused'
                            ? 'bg-primary text-[#050a14] shadow-[0_0_20px_var(--primary)]'
            ...
            <div className="flex flex-col xl:flex-row items-center gap-12 mb-10 relative z-10">
                ...
                <div className="relative w-36 h-36 flex-shrink-0">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                        ...
                        <span className="text-[8px] text-gray-600 font-black uppercase tracking-[0.3em] mt-1">Danger Lvl</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 flex-1 w-full">
                    {stats.map(s => (
                        <div key={s.label} className="glass-ethereal rounded-2xl p-5 border border-white/5 hover:border-primary/20 transition-all group/stat relative overflow-hidden">
            ...
            <div className="relative z-10">
                <p className="text-[9px] text-gray-600 mb-5 uppercase tracking-[0.5em] font-black italic opacity-60">✧ Safety Directives</p>
                <div className="space-y-4">
                    {data.recommendations.map((r, i) => (
                        <fm.div
                            ...
                            whileHover={{ x: 4 }}
                            className="flex items-start gap-4 p-4 rounded-xl bg-white/[0.01] border border-white/5 group hover:border-primary/30 hover:bg-white/[0.03] transition-all cursor-default"
                        >
